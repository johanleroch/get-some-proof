...
// `--install`, writes the site icons from the official icon in public/brand.
//
...
import { ICONS, iconFileName } from "./build.mjs";
...
/**
 * Site icons, all from the official app icon delivered by the founder
 * (public/brand/icon.svg, a Figma export: paper tile, upright blob, eyes):
 * - icon.svg: the file itself, served as the SVG favicon and read by
 *   `BrandMark` for the in-app mark.
 * - apple-icon.png: 180 px, square and opaque (the tile's corners are
 *   squared off because iOS masks the icon itself).
 * - favicon.ico: 16, 32 and 48 px PNG frames.
 */
async function installSiteIcons(browser) {
  const official = await readFile(
    path.join(ROOT, "public/brand/icon.svg"),
    "utf8",
  );
  await writeFile(path.join(APP_DIR, "icon.svg"), official, "utf8");
  console.log("installed src/app/icon.svg");

  const square = official.replace(/rx="280"/g, 'rx="0"');
  await writeFile(
  ...
  for (const size of [16, 32, 48]) {
    frames.push({
      size,
      png: await renderPng(browser, official, size, { transparent: true }),
    });
  }
...
function sheetHtml(svgs) {
  ...
  const row = (size, bg) =>
    `<section class="row ${bg}"><h2>${size}px</h2><div class="icons">${svgs
      ...
      .join("")}</div></section>`;
  const hero = (list) =>
    `<section class="hero">${list.map((s) => tile(s, 220, true)).join("")}</section>`;
  const series = [
    { title: "Series one, subjects", items: svgs.slice(0, 10) },
    {
      title: "Series two, mascots (the 08 blob's cousins)",
      items: svgs.slice(10),
    },
  ].filter((s) => s.items.length);
  return `<!doctype html><html><head><meta charset="utf-8"><style>
  body{margin:0;font:14px/1.4 "Figtree",system-ui,sans-serif;color:#2E2A25;background:#FCFAF5}
  main{padding:40px 48px 56px}
  h1{font:800 26px/1.1 "Gelica",system-ui,sans-serif;letter-spacing:-0.02em;margin:0 0 6px}
  ...
  .row h2{margin:0;font:600 13px/1 "Figtree",system-ui,sans-serif;letter-spacing:.06em;text-transform:uppercase;opacity:.6}
  ...
  h2.series{font:700 15px/1.2 "Gelica",system-ui,sans-serif;letter-spacing:-0.01em;margin:22px 0 10px}
  ...
  <p>Candidates in the appicons.store grammar, built on the brand tokens. Flagship: 03 quote marks. Check legibility at 64 and 32 px before choosing.</p>
  ${series.map((s) => `<h2 class="series">${s.title}</h2>${hero(s.items)}`).join("")}
  ${row(128, "paper")}
  ${row(64, "paper")}
  ${row(32, "paper")}
  ${row(128, "ink")}
