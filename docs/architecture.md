...
The server derives Pro Entitlement from synchronized Subscription state only when the synchronized Customer and Price match the server-owned `billingProfiles` mapping created during Checkout. `active` and `past_due` grant Pro; cancellation scheduled on one of those states grants it through the synchronized period end. Trialing, terminal, unpaid, incomplete, missing, unavailable, or mismatched states do not. The product capability boundaries independently enforce unlimited Pro text collection, 25 stored Ready videos, and automatic Promotion Card removal.
...
Before creating a direct upload, the Collection Form reads the encoded file's intrinsic width and height alongside its duration. Convex stores those source dimensions and their reduced aspect ratio on the Video Asset immediately, allowing the private Inbox to reserve the final card shape during `awaiting_upload` and `processing`. Mux metadata remains authoritative once received and may correct the aspect ratio before or at `Ready`.
