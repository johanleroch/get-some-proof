...
The server derives Pro Entitlement from synchronized Subscription state only when the synchronized Customer and Price match the server-owned `billingProfiles` mapping created during Checkout. `active` and `past_due` grant Pro; cancellation scheduled on one of those states grants it through the synchronized period end. Trialing, terminal, unpaid, incomplete, missing, unavailable, or mismatched states do not. The product capability boundaries independently enforce unlimited Pro text collection, 25 stored Ready videos, MP4 download, and automatic Promotion Card removal.
