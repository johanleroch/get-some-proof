...
| Platform Stripe Account and Organization Customer isolation                                            | ADR 0013; `tests/billingCheckout.test.ts`; `tests/billingManagement.test.ts`                                               |
| Server-only Stripe secrets, optional unavailable mode, and Free fallback                               | `docs/stripe-billing.md` safety boundary; `tests/billing.test.ts`; `tests/projectsPremium.test.ts`                         |
...
| Webhook-derived Premium states and cancellation-at-period-end date                                     | `convex/billingEntitlements.test.ts`; `tests/billingEntitlementOverview.test.ts`                                           |
| Free Project reads and role plus Premium checks on every Project write                                 | `tests/projectsPremium.test.ts`; `src/components/projects/project-manager.test.tsx`                                        |
...
| Forged Checkout success return never grants Premium                                                    | `src/components/billing/organization-billing.test.tsx`; `tests/projectsPremium.test.ts`; sandbox section 5.A               |
