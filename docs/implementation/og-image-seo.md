...
- A public 1200 × 630 PNG uses the official happy mascot, Gelica Black, Figtree and the paper/amber palette. Source: `scripts/brand/build-social-card.mjs`; regenerate with `node scripts/brand/build-social-card.mjs` after installing Playwright Chromium. The generator fetches the pinned Figtree font; serving the committed PNG needs no font fetch or backend.
