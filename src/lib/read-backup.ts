...
const testimonial = z.object({
  ...
  submissionType: z.enum(["text", "video"]),
...
const manifest = z.object({
  ...
  organization: z.object({ _id: z.string().min(1).max(128) }),
  testimonials: z.array(testimonial).max(10000),
  media: z
    .array(
      z.object({
        ownerId: z.string(),
...
export type BackupItem = {
  ...
  media: Array<{
    path: string;
    role: "avatar" | "thumbnail" | "image" | "video";
    entry: FileEntry;
  }>;
  missing: number;
};
...
async function extract(entry: FileEntry, maximum: number, type: string) {
  ...
  await entry.getData(
    new WritableStream<Uint8Array>({
      write(chunk) {
        ...
        if (size > maximum || size > entry.uncompressedSize)
          throw new Error("Backup file exceeds its size limit.");
...
export async function readBackup(file: Blob): Promise<BackupFile> {
  ...
  try {
    ...
    if (!json || json.uncompressedSize > 10 * 1024 ** 2)
      throw new Error(
        "Choose a Get Some Proof ZIP backup containing data.json.",
      );
    const data = manifest.parse(
      JSON.parse(
        await (await extract(json, 10 * 1024 ** 2, "application/json")).text(),
      ),
    );
    const byOwner = new Map<string, typeof data.media>();
    for (const asset of data.media) {
      const group = byOwner.get(asset.ownerId) ?? [];
      group.push(asset);
      byOwner.set(asset.ownerId, group);
    }
    const ids = new Set<string>();
    const items = data.testimonials.map((t) => {
      if (ids.has(t._id))
        throw new Error("Duplicate testimonial IDs in backup.");
      ids.add(t._id);
      ...
      for (const asset of data.media.filter(
        ...
        if (!safePath(asset.path) || paths.has(asset.path))
          throw new Error("Invalid media path in backup.");
        paths.add(asset.path);
        const entry = entries.get(asset.path);
        if (asset.status !== "included" || !entry) {
          missing++;
          continue;
        }
        const role =
          ...
          (asset.path.startsWith("videos/")
            ? "video"
            : /\/avatar\./.test(asset.path)
              ? "avatar"
              : /\/thumbnail\./.test(asset.path)
                ? "thumbnail"
                : "image");
        ...
        if (
          role === "video"
            ...
            : !asset.path.startsWith("images/") ||
              !/\.(png|webp|jpe?g|avif)$/.test(asset.path)
        )
          throw new Error("Unsupported backup media.");
        if (role !== "image" && media.some((m) => m.role === role))
          throw new Error("Duplicate media role.");
        media.push({
          path: asset.path,
          role: role as BackupItem["media"][number]["role"],
          entry,
        });
      }
      if (
        t.submissionType === "video" &&
        !media.some((m) => m.role === "video")
      )
        missing++;
      return {
        candidate: {
    ...
    return {
      sourceProject: data.organization._id,
      items,
      close: () => reader.close(),
    };
  } catch (error) {
...
export async function backupMediaBlob(media: BackupItem["media"][number]) {
  ...
  return extract(
    media.entry,
    media.role === "video" ? 512 * 1024 ** 2 : 25 * 1024 ** 2,
    type,
  );
}
