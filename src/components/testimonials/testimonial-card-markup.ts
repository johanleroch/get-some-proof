import { sourceIcons, sourceIconSvg } from "./source-icons";
import { testimonialSource } from "@convex/domain/testimonialSource";
...
/** The uploaded thumbnail when the Owner set one, otherwise a frame of the video. */
export function testimonialPoster(testimonial: TestimonialCardVideoValue) {
  if (testimonial.posterUrl) return testimonial.posterUrl;
...
function sourceMarkup(testimonial: TestimonialCardValue) {
  ...
  return source.url
    ...
    : `<span ${attributes} role="img">${icon}</span>`;
}
