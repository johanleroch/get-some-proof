...
export const expirePreview = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!job || job.expiresAt > Date.now() || (job.provider === "assistant" || job.provider === "backup"))
      return null;
...
export async function confirmOwnedImport(
  ...
  for (const id of new Set(args.itemIds)) {
    ...
    if (
      item.unavailableReason ||
      ((job.provider !== "assistant" && job.provider !== "backup") && item.type === "video" && !item.videoUrl)
    ) {
      result.unavailable++;
    ...
    if (item.type === "video") {
      if ((job.provider === "assistant" || job.provider === "backup") && !item.videoUrl) {
