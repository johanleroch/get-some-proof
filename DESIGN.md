...
- `Scribble star`: a loosely drawn five-point star, the logo's cousin. Marks
  the Wall header, the success step, and the 5-star rating in empty states.
- `Wavy underline`: a two-stroke wave under one key word in a display title.
...
provisional), a possible logo refresh once the palette is live, and whether the
Wall offers a "quiet" variant without the signature for very conservative
customer Brands.
